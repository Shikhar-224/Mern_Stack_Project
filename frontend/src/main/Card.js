...
const Card = ({product, showViewProductButton = true, showAddToCartButton = true, cartUpdate = false, showRemoveProductButton = false, setRun = f => f, run = undefined }) => {
  ...
  const showAddToCart = (showAddToCartButton) => {
    return (
        showAddToCartButton && (
        <button onClick = {addToCart} className = 'btn btn-outline-warning mt-2 mb-2 ml-4'>
            Add to Cart
        </button>
        )
    );
...
const showStock = quantity => {
  return quantity > 0 ? (
      ...
      <span className = 'badge badge-primary badge-pill ml-2'>Out Of Stock</span>
  );
};
        ...
        return (
          showRemoveProductButton && (
            <button
              ...
              className="btn btn-outline-danger mt-2 mb-2 ml-4">
              Remove Product
            </button>
          )
        );
